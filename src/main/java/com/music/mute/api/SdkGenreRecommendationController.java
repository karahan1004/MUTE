 ...
 * (accessToken != null) { try { spotifyApi.setAccessToken(accessToken);
 * 
 * // 현재 재생 중인 디바이스의 정보 가져오기 Device currentDevice =
 * getCurrentDevice(accessToken); model.addAttribute("currentDevice",
 * currentDevice);
 * 
 * // 음악 추천 목록 가져오기 List<TrackWithImageUrlVO> recommendations =
 ...
 * "redirect:/login"; } return "/sdkrecommendations"; }
 * 
 ...
 * 
 * CurrentlyPlayingContext playbackContext = null; try { playbackContext =
 ...
 * } } if (playbackContext != null) return playbackContext.getDevice(); else
 * return null; } catch (IOException | SpotifyWebApiException | ParseException
 * e) { e.printStackTrace(); // 예외를 다시 던져서 상위에서 처리하도록 함 throw e; } }
 * 
 * 
 * private Device getCurrentDevice(String accessToken) throws IOException,
